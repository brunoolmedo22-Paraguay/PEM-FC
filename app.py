...
tab_params, tab_sim, tab_results, tab_info, tab_audit, tab_repro = st.tabs(
...
# ======================================================================
# TAB 6 -- REPRODUCAO DA FIGURA 3 (Modo 2)
# ======================================================================
with tab_repro:
    st.subheader("Modo 2 -- Reproducao da Figura 3 (OTEKON 2024)")
    st.warning(
        "Os parametros abaixo NAO sao valores fornecidos pelo artigo. Sao "
        "a combinacao de convencoes e ajustes que melhor reproduz as curvas "
        "publicadas na Figura 3, obtida por engenharia reversa contra dados "
        "digitalizados da propria figura. Use a expressao **'interpretacao "
        "mais provavel compativel com os resultados publicados'** -- nunca "
        "'validado experimentalmente' ou 'valor do artigo'."
    )

    try:
        import sys as _sys
        _sys.path.insert(0, str(Path(__file__).resolve().parent))
        from calibration.flexible_model import Conventions as _Conv, nernst_E as _nernst_E, \
            oxygen_concentration as _o2, activation_loss as _vact
        from calibration.load_digitized import load_all as _load_all
        import numpy as _np
        import matplotlib
        matplotlib.use("Agg")
        ...
        XI1_REPRO = -0.63682
        RMEM_REPRO = 0.20321   # Ohm.cm2, ajustado -- bypassa Eq. 11/12
        NA_REPRO = 9477.4      # N_cells x A_active, cm2
        A_ACTIVE_REPRO = 232.0
        N_CELLS_REPRO = NA_REPRO / A_ACTIVE_REPRO
        C_EFF_REPRO = 0.55331  # eta = C x Vcell (ajuste empirico)

        c1, c2, c3 = st.columns(3)
        c1.metric("xi1 (ativacao)", f"{XI1_REPRO:.4f}", "literal: -0.9514")
        c2.metric("R_mem (ohmica, livre)", f"{RMEM_REPRO:.3f} Ohm.cm2",
                  "literal: ~1.85e-6 kOhm.cm2 (desprezivel)")
        c3.metric("N_cells (estimado)", f"{N_CELLS_REPRO:.1f}", f"A={A_ACTIVE_REPRO:.0f} cm2")

        st.markdown(
            f"**Pressoes (da legenda da Figura 3, painel 'Activation Loss' "
            f"inferior-direito):** p_H2 = 1.0 atm, P_total catodo "
            f"(rotulado 'P_hava' = P_ar no original turco) = 5.0 atm, "
            f"p_O2 = 0.21 x 5.0 = 1.05 atm (fracao molar padrao assumida).  "
            f"**Eficiencia:** ajuste empirico eta = {C_EFF_REPRO*100:.1f}% x "
            f"V_cell (nao corresponde precisamente a Vcell/E, /1.229, "
            f"/1.254 nem /1.481 isoladamente)."
        )

        conv = _Conv(pressure_source="figure", xi1=XI1_REPRO, alpha_H=0.0)

        def _vcell(x, T):
            E, p_o2v = _nernst_E(T, conv)
            c_o2 = _o2(T, p_o2v)
            vact = _vact(x, T, c_o2, conv)
            return E - vact - x * RMEM_REPRO

        DATA = _load_all()
        x_plot = _np.linspace(0.001, 1.15, 200)
        colors = {298.15: "#1f5f8b", 373.15: "#c0392b"}

        fig, axes = _plt.subplots(2, 2, figsize=(11, 8.5))

        ax = axes[0, 0]
        for T in [298.15, 373.15]:
            xd, yd = DATA[("panel4", T)]
            ax.scatter(xd, yd, s=10, color=colors[T], alpha=0.5)
            ax.plot(x_plot, _vcell(x_plot, T), "-", color=colors[T], lw=2)
        ax.set_title("Tensao (paineis rotulados 'Activation Loss (V)')")
        ax.set_xlabel("i [A/cm2]"); ax.set_ylabel("V"); ax.grid(alpha=0.3)

        ax = axes[0, 1]
        for T in [298.15, 373.15]:
            xd, yd = DATA[("panel2_power", T)]
            ax.scatter(xd, yd, s=10, color=colors[T], alpha=0.5)
            ax.plot(x_plot, _vcell(x_plot, T) * x_plot * NA_REPRO, "-", color=colors[T], lw=2)
        ax.set_title("Potencia"); ax.set_xlabel("i [A/cm2]"); ax.set_ylabel("W"); ax.grid(alpha=0.3)

        ax = axes[1, 0]
        for T in [298.15, 373.15]:
            xd, yd = DATA[("panel3_efficiency", T)]
            ax.scatter(xd, yd, s=10, color=colors[T], alpha=0.5)
            ax.plot(x_plot, 100 * C_EFF_REPRO * _vcell(x_plot, T), "-", color=colors[T], lw=2)
        ax.set_title("Eficiencia"); ax.set_xlabel("i [A/cm2]"); ax.set_ylabel("%"); ax.grid(alpha=0.3)

        ax = axes[1, 1]
        for T in [298.15, 373.15]:
            xd, yd = DATA[("panel4", T)]
            ax.scatter(xd, _vcell(xd, T) - yd, s=10, color=colors[T])
        ax.axhline(0, color="black", lw=1)
        ax.set_title("Residuos (tensao)"); ax.set_xlabel("i [A/cm2]"); ax.set_ylabel("V"); ax.grid(alpha=0.3)

        _plt.tight_layout()
        st.pyplot(fig)

        st.caption(
            "Pontos = dados digitalizados pixel a pixel da Figura 3 original "
            "(azul=298.15K, vermelho=373.15K). Linhas = modelo de reproducao. "
            "Metricas completas em data/otekon_figure3/metricas_finais.csv."
        )
    except Exception as e:
        st.error(f"Nao foi possivel carregar o modo de reproducao: {e}")
